...
export function createAndSelectTableOrderLocal(tableNumber, tableSubNumber) {
  return (dispatch, getState) => {
    ...
    if (
      createdOrder.orderType !== "table" ||
      createdOrder.tableInfo.tableNumber !== tableNumber ||
      createdOrder.tableInfo.tableSubNumber !== tableSubNumber
    ) {
      let index = newState.findIndex(
        (order) =>
          order.orderType === "table" &&
          tableNumber === order.tableInfo.tableNumber &&
          tableSubNumber === order.tableInfo.tableSubNumber
      );
  ...
  return (dispatch, getState) => {
    ...
    fetch("https://nhatranghk.site/orderActions.php?function=newNTOrder", {
      ...
      .then((responseJson) => {
        ...
        var createdOrder = responseJson.filter(
          (i) => i.orderNumber === orderNumber
        );
...
export function createAndSelectDeliveryOrder(
  deliveryAddress,
  deliveryPhoneNumber
) {
  return (dispatch, getState) => {
...
export function createAndSelectDeliveryOrderLocal(
  deliveryAddress,
  deliveryPhoneNumber
) {
  return (dispatch, getState) => {
    ...
    if (
      ...
      createdOrder.deliveryInfo.deliveryAddress !== deliveryAddress ||
      createdOrder.deliveryInfo.deliveryPhoneNumber !== deliveryPhoneNumber
    ) {
      let index = newState.findIndex(
        (order) =>
          order.orderType === "delivery" &&
          deliveryAddress === order.deliveryInfo.deliveryAddress &&
          deliveryPhoneNumber === order.deliveryInfo.deliveryPhoneNumber
      );
      createdOrder = newState[index];
    }
    dispatch(selectOrder(createdOrder));
...
export function findOrderAndAddDrinkItem(drinkItem, orderNumber) {
  return (dispatch, getState) => {
    var currentOrder = getState().orders.filter(
      (i) => i.orderNumber === orderNumber
    );
    var newNumOfItems =
      currentOrder[0].orderInfo.numOfItems + drinkItem.quantity;
    var newTotalPrice =
      currentOrder[0].orderInfo.totalPrice + drinkItem.drink.price;
...
export function findOrderAndAddFoodItem(foodItem, orderNumber) {
  return (dispatch, getState) => {
    ...
    var newNumOfItems =
      currentOrder[0].orderInfo.numOfItems + foodItem.quantity;
    var newTotalPrice =
      currentOrder[0].orderInfo.totalPrice + foodItem.ingredients[0].price;
  ...
  return (dispatch, getState) => {
    ...
    fetch("https://nhatranghk.site/orderActions.php?function=addMealItem", {
      ...
      body: JSON.stringify({
        ...
        ingredient1: foodItem.ingredients[0].name,
        ingredient2: foodItem.ingredients[1]
          ? foodItem.ingredients[1].name
          : "",
        ingredient3: foodItem.ingredients[2]
          ? foodItem.ingredients[2].name
          : "",
        ingredient4: foodItem.ingredients[3]
          ? foodItem.ingredients[3].name
          : "",
      }),
    })
      .then((response) => response.json())
...
export function findOrderAndAddMealAndDrinkItem(
  foodItem,
  drinkItem,
  orderNumber
) {
  return (dispatch, getState) => {
    ...
    mealPrice += foodItem.base.price;
    var newNumOfItems =
      currentOrder[0].orderInfo.numOfItems +
      foodItem.quantity +
      drinkItem.quantity;
    var newTotalPrice =
      currentOrder[0].orderInfo.totalPrice + mealPrice + drinkItem.drink.price;
    fetch(
      "https://nhatranghk.site/orderActions.php?function=addMealAndDrinkItem",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (dispatch) => {
    fetch("https://nhatranghk.site/orderActions.php?function=payOrder", {
      ...
      body: JSON.stringify({
        ...
        endTime: getTime(),
        orderList: [
          ...order.orderList.drinkItems,
          ...order.orderList.foodItems,
        ],
      }),
    })
      .then((response) => response.json())
